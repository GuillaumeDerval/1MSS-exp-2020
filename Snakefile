...
def produce_synthetic_small_sql_gen_ids():
    ...
    for size in config["synth_small_sizes"]:
        for mean, sigma in config["synth_small_fill_with"]:
            for methodDirect, methodTranspose in config["ctr_methods"]:
                for branching in config["branch_methods"]:
                    for dataset_id in range(config["synth_small_n_instances_per_size"]):
                        out.append(f"{size}_{mean}_{sigma}_{dataset_id}_{methodDirect}_{methodTranspose}_{branching}")
    return out
...
rule produce_synthetic_small_sql:
    ...
    output:
        "results/synthetic_small.db"
    params:
        format=r"(?:.*[^0-9])?(?P<size>[0-9]+)_(?P<mean>[\-0-9\.]+)_(?P<sigma>[0-9\.]+)_(?P<dataset_id>[0-9]+)_(?P<ctr_direct>[^_]+)_(?P<ctr_transpose>[^_]+)_(?P<branching>[^_]+)\.txt",
        params=[("size", int), ("mean", float), ("sigma", float), ("dataset_id", int), ("ctr_direct", str), ("ctr_transpose", str), ('branching', str)]
    script:
...
rule start_synthetic_small:
    input:
        ...
        jar=config["jarpath"]
    output: "results/synthetic_small/{dataset}_{ctr_direct}_{ctr_transpose}_{branching}.txt"
    group: "synth_{dataset}"
    shell: "java -cp {input.jar} oscar1mss.runners.CompleteRunner {input.file} 7200 {wildcards.ctr_direct} {wildcards.ctr_transpose} {wildcards.branching} > {output}"
...
def produce_synthetic_big_sql_gen_ids():
    ...
    for size, psol, timeout, first_n_instances_considered in config["synth_big_sizes"]:
        for method_direct, method_transpose in config["ctr_methods"]:
            for branching in config["branch_methods"]:
                for dataset in range(first_n_instances_considered):
                    out.append(f"{size}_{psol}_{dataset}_{method_direct}_{method_transpose}_{branching}_{timeout}")
    return out
...
rule produce_synthetic_big_sql:
    ...
    params:
        ...
        params=[("n_rows", int), ("n_cols", int), ("p_sol", int), ("dataset_id", int), ("ctr_direct", str), ("ctr_direct", str), ('branching', str), ('max_time', int)]
    script:
        "scripts/run_to_sqlite.py"
